...
// over it. The parity worklist expects 68 tools (incl. the agentTask.status /
// agentTask.list readers); we assert that exact count so a silent family add/drop
// is caught.
func TestCreateRegistersFullToolSet(t *testing.T) {
	...
	if got != 68 {
		t.Errorf("registered tools = %d, want 68", got)
	}
