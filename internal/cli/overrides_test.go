...
// TestAPIKeyFileAppliesKeyShape: the same structural check as the login prompt, so a
// stray newline or smart quote becomes a readable message here instead of an opaque 401.
...
// a fall-through to the stored sign-in. Falling back would run the job against a
