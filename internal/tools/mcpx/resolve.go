...
//
// ATOMIC on a definitive miss: if the roster IS readable and ANY id resolves to
// nothing (or to several terminals), the whole call fails with zero moves. A partial
// move is the worst outcome — the model asked to relocate a cohort, and half a cohort
// in the destination with no signal about which half is harder to recover from than a
// clean rejection naming the live ids.
//
