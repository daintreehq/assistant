...
//
// The stakes changed when the sign-in went away but did not go down. A run needs NO key
// now — the backend funds it — so naming one is a deliberate choice to spend a specific
// account's credit, and falling back to anything else bills the wrong one. Each case sets
// DAINTREE_API_KEY to a working-looking decoy; the assertions below key on stderr naming
// --api-key-file and the offending path, which only the key-file path can produce, so a
// silent fallback would fail them even though it too would exit non-zero.
...
// TestKeyFileRejectsMalformedKey: the file path applies the SAME structural check
// DAINTREE_API_KEY gets at config load, so a stray newline or a pasted-in wrapper becomes
// a readable message here rather than an opaque header error on every turn.
