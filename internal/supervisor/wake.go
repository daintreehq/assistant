...
// but dispatch as ActorWake (grants gate mutations); they are unabortable by
// design except through the supervision context (an attach handover cancels
// sctx, which unwinds the in-flight Send).
func (r *Runtime) reactWake(ctx context.Context) {
	r.mu.Lock()
	a := r.app
	if a == nil || r.wakeBusy || len(r.pendingWake) == 0 {
		r.mu.Unlock()
		return
	}
	...
	func() {
		...
		if ctx.Err() != nil {
			return // supervision already ending — requeue below via wakeBusy reset
		}
	...
	r.mu.Lock()
	r.wakeBusy = false
	more := len(r.pendingWake) > 0 && ctx.Err() == nil
	r.mu.Unlock()
	if more {
		go r.reactWake(ctx)
	}
