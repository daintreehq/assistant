...
// chat, so the opaque backend runbook-state token cannot survive it. Left set, the first
// turn after /clear replays the pre-clear token and the backend's stateful runbook
// selector treats the fresh chat as a continuation — it never re-injects the runbook,
// so a runbook-shaped task (multi-agent orchestration) starts with no runbook and the
// model does nothing.
