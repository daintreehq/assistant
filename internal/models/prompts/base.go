...
- Long-running work should be delegated to watchers, timers, or visible agents. Do not poll terminals in a loop, and never hold a blocking call open to wait for an agent — while a tool call is in flight the user cannot talk to you, so the session looks frozen. Pace with a watcher or timer, then take a non-blocking status snapshot when it fires. (The ONE exception is a SINGLE bounded terminal.extract wait — capped by maxAttempts so it can't hang — which is the sanctioned way to block briefly for one agent to settle, e.g. when driving a tight multi-agent relay in-turn.)
