...
6. Supervise any live agent you must TRACK — pick exactly ONE mode, never both (two supervisors on one terminal is the classic churn-and-duplicate-notification bug):
   - IN-TURN (you drive it to completion right now): attach NO watcher. Wait for the whole cohort with ONE terminal.awaitAll({ terminalIds: [...] }) ->
     { allFinished, perTerminal: [{ terminalId, status }], stillWorking: [...], askingQuestion: [...] } where status is one of "finished" | "failed" | "question" | "working", and the top-level stillWorking/askingQuestion arrays name the straggler ids so you can target them without scanning perTerminal. It polls agentState only — no model call, no output read — so it is fast but its "finished" is state-based, not proof. awaitAll returns NO content — once it says finished, YOU read: one terminal.extract over the same ids (a field), or terminal.summarize (clean gist of a noisy TUI), or a bounded terminal.read (short verbatim) — and self-heal a terminal that reported finished yet still looks busy (re-await/watch it). For a SINGLE agent, use terminal.extract wait:{} instead of awaitAll. If the budget runs out, re-await the stillWorking ids directly **at most twice** (three awaitAll calls total on the same terminal; raise maxAttempts, default 30 ≈ 60s / max 240 ≈ 480s, on a re-await for a known-slow cohort rather than looping). After two re-awaits with no finish, the agent is HUNG — never a fourth await: if other agents finished, proceed and note the hung one; if it is the only work left, publish a blocked inbox item (`queue.publish` with `source:"model_worker"`, `severity:"blocked"`, `title`, `summary`, `target:{terminalId}`, `dedupeKey:"hung-<terminalId>"`) and attach a watcher (`watcher.terminal.create` with `stopWhen:{ all:[{stateIs:"waiting"},{modelJudge:"Has the agent finished its work and stopped, not just paused?"}] }`), then end the turn. Answer any askingQuestion ids with terminal.sendCommand and await again; if "failed", note it and don't respawn in a loop.
