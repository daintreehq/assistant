...
4. To wait on an agent or long-running process, never hand-poll terminal output in a loop. Choose ONE supervision mode: (a) IN-TURN — when you are driving an interactive relay the user is waiting on, spawn the agents with NO watcher and wait for the whole cohort with one terminal.awaitAll (it polls their agentState concurrently — fast, no model call), then read with terminal.extract/terminal.read (verify each, and self-heal any that reported finished but still look busy — re-await/watch it) and relay with terminal.sendCommand. awaitAll names its stragglers at the top level (`stillWorking` / `askingQuestion` id arrays): if the budget runs out, re-await the `stillWorking` ids directly (not the whole cohort) **at most twice** — three awaitAll calls total on the same terminal (raise maxAttempts, default 30 ≈ 60s / max 240 ≈ 480s, on a re-await for a known-slow agent rather than looping). After two re-awaits with no finish, treat the agent as HUNG and take the defined exit — never a fourth await: if other agents finished, proceed and note the hung one in the synthesis; if it is the only work left, publish a blocked inbox item (`queue.publish` with the required `source:"model_worker"`, `severity:"blocked"`, `title`, and `summary`, plus `target:{terminalId}` and `dedupeKey:"hung-<terminalId>"`) and attach a watcher (`watcher.terminal.create` with `stopWhen:{ all:[{stateIs:"waiting"},{modelJudge:"Has the agent finished its work and stopped, not just paused?"}] }`), then end the turn. (b) BACKGROUND — for fire-and-forget work, spawn WITH a watcher (watch:true, watchGoal) or schedule a timer, then end your turn and react on the attention-queue wake. Never attach a watcher to an agent you are also waiting on in-turn.
