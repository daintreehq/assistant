...
4. Use agentTask.spawnForEdits — NEVER a raw agent.launch via daintree.call — with: mode ("edit" to change files, "explore" for a read-only investigation), title (short task name — becomes the spawned agent's visible name/tab label in Daintree, so keep it distinct when running several in parallel), taskPrompt (exact instructions), context.filePaths when known, and — unless the task is trivial — a nested watcher object `watcher: {"create": true, "goal": "..."}` (a nested object, NOT a flattened/dotted `watcher.create` key, which the strict decoder rejects as an unknown field).
