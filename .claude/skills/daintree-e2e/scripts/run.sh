...
# (env DAINTREE_MCP_TOKEN, or the newest ~/.daintree/logs mcp.credentials line).
...
# 2. fresh MCP creds — env override wins, else newest log
IFS=$'\t' read -r MCP_URL MCP_TOKEN < <("$PY" "$HERE/mcp.py" creds)
[ -n "${MCP_TOKEN:-}" ] || { echo "no MCP token (open a Daintree session)" >&2; exit 1; }
