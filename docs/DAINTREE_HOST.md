...
| `DAINTREE_UPSTREAM_PROVIDER`, `DAINTREE_UPSTREAM_API_KEY`, `DAINTREE_UPSTREAM_MODEL` | Bring your own key: the model provider (`baseten`, `openrouter` or `openai`), the user's key for it, and — only when the user overrode the recommendation — the model, all from Daintree's assistant settings. Set only when a key is saved; a provider without a key fails startup. Forwarded to the backend as `X-Daintree-Upstream-*` on every call (docs/BACKEND.md). | **Yes** → `cfg.Upstream` |
| `DAINTREE_UPSTREAM_SORT`, `DAINTREE_UPSTREAM_DATA_COLLECTION`, `DAINTREE_UPSTREAM_ZDR` | OpenRouter only: rank endpoints by `latency`, `price` or `throughput`; `deny` to skip endpoints that collect or train on prompts; `true` to keep to endpoints OpenRouter lists as zero data retention. Refused at startup with any other provider. | **Yes** → `cfg.Upstream` |
...
| `timers:snapshot` | `timers[]` (the same row shape as `operations:snapshot`), `outcomes[]`, `takenAt`, `readFailed` — the answer to one inbound `timers`. `outcomes[]` (`eventId`, `timerId`, `severity`, `title`, `summary`, `createdAt`, `updatedAt`, `count`) is what recently-fired timers DID — a **different dimension** from the schedule rows, because a timer's status going to `fired` is not success: the scheduler claims and advances the row before running the payload. `count` is how many firings one row stands for (a repeat publishes under one dedupe key, so the twelfth failure updates the first row). **`readFailed` is not cosmetic**: the operations deck is best-effort and drops a section it could not load, but a timer manager must never render a failed read as "nothing scheduled", because that is a claim a user acts on by walking away from work that is still queued. An empty `timers[]` means nothing while `readFailed` is true |
...
| `timer:fired` | `timerId`, `firedAt` — **pushed**, unprompted, when a timer fires. An INVALIDATION, not a payload: re-read `timers` rather than patching a row from it. This is the only unsolicited timer signal, and it exists because there was none — a timer's own fire never wakes the assistant (by design: a reminder is for a human, not a prompt) and a successful tool call publishes below the attention threshold, so a timer could fire and a host learn nothing. **v4.** |
