...
| `DAINTREE_UPSTREAM_PROVIDER` / `_MODEL` / `_API_KEY` → `cfg.Upstream` → `X-Daintree-Upstream-*` | set by Daintree when the user picks a provider in its assistant settings | bring your own key: names the caller's own model host (`baseten`, `openrouter` or `openai`), model (empty → the backend's recommendation) and key. The backend then runs the turn's model calls, and its utility tasks, on that key instead of its own, with no entitlement check. Trusted env only, validated at startup, never the bearer, scrubbed from human-facing text, and dropped when an MCP session redirects the backend URL. With `openrouter`, `DAINTREE_UPSTREAM_SORT` / `_DATA_COLLECTION` / `_ZDR` add OpenRouter's own routing preferences as headers. A rejected caller key arrives with `param: X-Daintree-Upstream-Key`, and the turn shows the backend's own message rather than the deployment-key copy |
