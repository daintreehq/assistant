...
- `agentTask.spawnForEdits` — Args `{worktreeId?: string, agentId?: string (default "claude"), title, taskPrompt, context?: {filePaths?: string[], includeDiff?: boolean}, watcher?: {create: boolean, goal?: string, cadenceMs?: number}}`. Behavior: build an agent prompt (compose taskPrompt + the standard constraints block from docs §18: "Make changes only in this worktree… Report back changed files/tests/risks"), then `ctx.mcp.callTool("agent.launch", {agentId, worktreeId?, prompt})` (requestKey via randomUUID). From the result, get the terminalId (structuredContent.terminalId ?? parse). If `watcher.create`, insert a terminal watcher (same shape as watcher.terminal.create) for that terminalId. Return `{terminalId, worktreeId, watcherId?}`. If MCP down, fail with guidance. This tool embodies the rule: the CLI never edits files; it spawns a visible agent.
